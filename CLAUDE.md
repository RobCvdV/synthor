...
- **The sequencer is C++ — `src/native/TxSeq.h`, registered in `src/native/Main.cpp`.** Changing it changes nothing until `scripts/build-elementary-wasm.sh` rebuilds the wasm and the renderer packages (node_modules is patched from `vendor/elementary`, pinned at the web/offline v4.0.3 commit). Restart the dev server after a rebuild — vite caches the old dep bundle.
