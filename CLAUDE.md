...
- **Three SchedulerNodes** — browsers cap AudioWorkletNode at 32 channels; the scheduler spans 3×32 control channels. The Elementary node must declare `numberOfInputs: 4` with only 3 inputs connected: declaring 3 inputs OR connecting a 4th source both silence every input (Chromium bug, reproduced on Electron 35/43 and Chrome).
